...
description: Choose the right junior mentoring or delegation mode for a task and route work into copy-along, spec-coach, review-mode, work-order, code-assignment, spec-assignment, env-task-sheet, bugfix-brief, refactor-ticket, or test-task. Use when the user wants one entry point for training or assigning work to a junior developer.
---

# Junior Dev Orchestrator

이 스킬은 주니어 교육 및 위임용 상위 라우터입니다. 사용자의 상황을 보고 아래 모드 중 가장 맞는 것을 고르거나, 필요하면 2단계 흐름으로 이어줍니다.
...
- 사용자가 "주니어 교육 모드로 진행해줘" 또는 "주니어에게 넘길 문서를 써줘"처럼 넓게 요청할 때
- 어떤 교육 모드가 맞는지 아직 정하지 않았을 때
- AI는 계속 일을 하되, 주니어 교육 방식이나 위임 문서 방식을 상황에 맞게 자동 선택하고 싶을 때
...
- 주니어에게 실제 업무 티켓처럼 작업을 넘겨야 함
- AI는 구현하지 않고 작업지시서만 써야 함

### 5. Code Assignment 선택

- 주니어가 직접 입력할 코드 조각과 명령어가 필요함
- 따라 치기보다는 실무 과제 문서가 필요함

### 6. Spec Assignment 선택

- 구현 방향과 완료 기준만 주고 직접 구현하게 해야 함
- 교육형 spec이 아니라 실무형 assignment가 필요함

### 7. Env Task Sheet 선택

- 설치, 실행, 빌드, 테스트, 환경 변수 준비를 주니어가 맡아야 함

### 8. Bugfix Brief 선택

- 버그 재현, 조사, 수정 작업을 주니어에게 넘겨야 함

### 9. Refactor Ticket 선택

- 기능 변경보다 구조 개선 작업을 위임해야 함

### 10. Test Task 선택

- 테스트 작성 업무를 별도 과제로 넘겨야 함
...
### 흐름 C. Work Order → Review Mode

실무 작업을 주니어에게 넘기고, 완료 후 교육적으로 리뷰하는 흐름입니다.

### 흐름 D. Spec Assignment → Review Mode

정답 코드는 주지 않고 직접 구현하게 한 뒤, 결과물을 리뷰하는 흐름입니다.

### 흐름 E. Env Task Sheet → Bugfix Brief

환경을 먼저 주니어가 올리고, 그다음 재현/수정 과제로 이어가는 흐름입니다.
...
## 선택 이유
- 현재 요청은 주니어가 실제 실무를 수행할 수 있는 작업 문서가 필요함
...
- "주니어에게 바로 넘길 작업지시서 문서를 어떤 형식으로 써야 할지 골라줘"
- "버그 수정 과제, 테스트 과제, 환경 세팅 과제 중 뭘 써야 하는지 판단해줘"
